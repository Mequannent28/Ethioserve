...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_bus'])) {
    if (verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            if (!$user) {
                $username = 'bus_' . strtolower(str_replace(' ', '', $company_name)) . rand(10, 99);
            ...
            $pdo->commit();
            redirectWithMessage('view_bus.php?id=' . $company_id, 'success', 'Bus company registered successfully');
        } catch (Exception $e) {
            $pdo->rollBack();
...
<head>
    ...
    <title>Add Bus Company - Admin Panel</title>
        ...
        <div class="main-content">
            <div class="container-fluid py-4">
                <div class="d-flex justify-content-between align-items-center mb-5">
                    <div>
                        <h2 class="fw-bold mb-1">Add Bus Company</h2>
                        <p class="text-muted mb-0">Manually register a new bus service provider.</p>
                    </div>
                    <a href="manage_bus.php" class="btn btn-outline-secondary rounded-pill px-4">
                        ...
                        <div class="row g-4">
                            <div class="col-md-12">
                                ...
                                <input type="text" name="company_name" class="form-control"
                                    placeholder="e.g. Cross Country Express" required>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-muted uppercase">Location / Terminal</label>
                                <input type="text" name="location" class="form-control"
                                    placeholder="e.g. Merkato Terminal, Addis Ababa" required>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="email" name="owner_email" class="form-control"
                                    placeholder="owner@buscompany.com" required>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" name="temp_password" class="form-control" value="bus123">
                            </div>
                        </div>
                    </div>

                    <div class="d-flex justify-content-end mt-4">
                        <button type="submit" class="btn btn-bus rounded-pill shadow-sm">
                            <i class="fas fa-bus me-2"></i>Register Bus Company
                        </button>
                    </div>
                </form>
            </div>
