    ...
    if (verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            if (!$user) {
                ...
                $user_id = $user['id'];
                $stmt = $pdo->prepare("UPDATE users SET role = 'restaurant' WHERE id = ?");
                $stmt->execute([$user_id]);
            }

            // Create Restaurant
            $stmt = $pdo->prepare("INSERT INTO restaurants (user_id, name, location, cuisine_type, status, rating) VALUES (?, ?, ?, ?, 'approved', 4.5)");
            $stmt->execute([$user_id, $name, $location, $cuisine]);
            $restaurant_id = $pdo->lastInsertId();

            $pdo->commit();
            redirectWithMessage('view_restaurant.php?id=' . $restaurant_id, 'success', 'Restaurant registered successfully');
        } catch (Exception $e) {
            $pdo->rollBack();
        ...
        <div class="main-content">
            <div class="container-fluid py-4">
                <div class="d-flex justify-content-between align-items-center mb-5">
                    <div>
                        <h2 class="fw-bold mb-1">Add New Restaurant</h2>
                        <p class="text-muted mb-0">Manually register a new restaurant and its owner account.</p>
                    </div>
                    <a href="manage_restaurants.php" class="btn btn-outline-secondary rounded-pill px-4">
                ...
                <form method="POST" class="form-container">
                    ...
                    <div class="form-section">
                        ...
                        <div class="row g-4">
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-muted uppercase">Restaurant Name</label>
                                <input type="text" name="name" class="form-control" placeholder="e.g. Traditional Taste"
                                    required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted uppercase">Location</label>
                                <input type="text" name="location" class="form-control"
                                    placeholder="e.g. Piazza, Addis Ababa" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted uppercase">Cuisine Type</label>
                    ...
                    <div class="form-section">
                        <h5 class="section-title">Proprietor Information</h5>
                        <div class="row g-4">
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-muted uppercase">Owner Full Name</label>
                                <input type="text" name="owner_name" class="form-control"
                                    placeholder="Full name of the owner" required>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted uppercase">Phone Number</label>
                    ...
                    <div class="d-flex gap-3 justify-content-end mt-4">
                        <button type="submit" class="btn btn-primary-green rounded-pill px-5 py-3 fw-bold shadow-sm">
                            <i class="fas fa-plus-circle me-2"></i>Register Restaurant
                        </button>
                    </div>
                </form>
            </div>
