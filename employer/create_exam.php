...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_exam'])) {
    ...
    if ($stmt->execute([$company_id, $title, $description, $duration])) {
...
<head>
    ...
    <title>Create Exam - EthioServe</title>
    ...
    <div class="dashboard-wrapper d-flex">
        ...
        <div class="main-content flex-grow-1">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="fw-bold"><i class="fas fa-file-alt me-2 text-primary"></i>Create New Exam</h2>
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-8">
                                ...
                                <input type="text" name="title" class="form-control" required placeholder="e.g. PHP Developer Assessment">
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" class="form-control" rows="3" placeholder="Instructions for candidates..."></textarea>
                        </div>
                        ...
                        <h5 class="fw-bold mb-3 d-flex justify-content-between align-items-center">
                            ...
                            <div class="btn-group">
                                ...
                                <button type="button" class="btn btn-sm btn-warning rounded-end-pill px-3 fw-bold" data-bs-toggle="modal" data-bs-target="#importModal"><i class="fas fa-file-import me-1"></i> PDF/Word Import</button>
                            </div>
                        </h5>
...
Answer: B"></textarea>
                        ...
                        <div id="questionsContainer">
                            ...
                            <div class="question-block" data-index="0">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold text-primary">Question 1</label>
                                    <textarea name="questions[0][text]" class="form-control" rows="2" required placeholder="Enter question text..."></textarea>
                                </div>
                                <div class="row g-3 mb-3">
                                    <div class="col-md-6">
                                        <div class="input-group">
                                            <span class="input-group-text">A</span>
                                            <input type="text" name="questions[0][opt_a]" class="form-control" required placeholder="Option A">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="input-group">
                                            <span class="input-group-text">B</span>
                                            <input type="text" name="questions[0][opt_b]" class="form-control" required placeholder="Option B">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="input-group">
                                            <span class="input-group-text">C</span>
                                            <input type="text" name="questions[0][opt_c]" class="form-control" placeholder="Option C (Optional)">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="input-group">
                                            <span class="input-group-text">D</span>
                                            <input type="text" name="questions[0][opt_d]" class="form-control" placeholder="Option D (Optional)">
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    <label class="form-label fw-bold">Correct Option *</label>
                                    <select name="questions[0][correct]" class="form-select w-auto" required>
                        ...
                        <div class="text-end mt-4">
                            <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow-sm"><i class="fas fa-save me-2"></i>Save Exam</button>
                        </div>
                    </form>
                </div>
            </div>
...
Answer: A"></textarea>
    ...
    <script>
        ...
        function addQuestionBlock(data = null) {
            ...
            // If data is provided, use its values, otherwise use defaults
        ...
        function removeInitialIfEmpty() {
            const firstBlock = document.querySelector('.question-block[data-index="0"]');
            if (firstBlock) {
        ...
        /**
         * Robust Parser for questions
         * Handles many formats including:
         * 1. Q text?
         * A) Opt...
         * Answer: A
         */
        function parseQuestions(text) {
            ...
            lines.forEach(line => {
                ...
                // Matches: "1. text", "1) text", "(1) text", "Question 1: text", "1: text"
                const qMatch = line.match(/^(?:(?:\(|Question\s+)?(\d+)[.)\]:]\s*|([?¿]))\s*(.*)/i);
                
                // Also handle cases where question starts with text but no number if we don't have a current question
                // OR if the line doesn't look like an option/answer and we just finished a question.
                if (qMatch) {
                ...
                if (currentQ) {
                    ...
                    // 3. Detect Answer
                    // Matches: "Answer: A", "Ans: A", "Correct: A", "Key: A", "Answer is A", "[A]"
                    ...
                    // 4. Continuation of question text (if no options found yet)
                    if (!currentQ.opt_a) {
                        currentQ.text += " " + line;
                    }
                } else if (!currentQ && line.length > 10) {
                    // Start a new question if we see significant text and don't have one active
